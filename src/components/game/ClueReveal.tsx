...
const ClueReveal: React.FC<ClueRevealProps> = ({ clues, cluesRevealed, maxClues, onRevealClue }) => {
      ...
      <ul className="space-y-4 w-full">
        {clues.slice(0, cluesRevealed).map((clue, idx, arr) => {
          ...
          return (
            ...
            >
              <span className="inline-block font-heading font-semibold text-base sm:text-lg rounded-lg px-3 py-1 bg-accent text-white mr-2 shadow-sm transition-all duration-200" style={{fontWeight: 600}}>
                {idx + 1}
              </span>
              <span className="font-body text-base sm:text-lg text-neutral-700 leading-relaxed transition-all duration-200" style={{fontWeight: 400}}>
                {clue}
              </span>
            </li>
          );
